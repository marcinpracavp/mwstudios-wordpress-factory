...
function prompt(stage, task, runDir, feedback = '') {
  ...
  const common = `You are one sequential worker in MWStudios Website Factory Autopilot.
...
Work autonomously in THIS theme and its own WordPress instance only: ${project.environment.localUrl}.
...
The actual current project is ${project.project.name}. Ignore example brand names, dimensions and languages in legacy docs.
...
CURRENT ACCEPTANCE: ${(visual.maxDifferentPixelRatio * 100).toFixed(1)}% pixel difference per ownership group: page, header, footer, shared components.
...
Run directory: ${path.relative(ROOT, runDir).replaceAll('\\', '/')}.
Task: ${JSON.stringify(task)}.
${feedback ? `Previous attempt/gate feedback (act only on these concrete failures):\n${feedback}\n` : ''}
`;
  const stages = {
